...
def chat_join(message):
    ...
    if NOTIFY_USERS_ON_ENTER_OR_LEAVE_ROOMS:
        room.send_message(None, message.user, datetime.now().isoformat(), MSG_TYPE_ENTER)
    ...
    messages_data = Message.objects.filter(room__id=message['room']).order_by('-time')[:30]
    messages = []
    for message_data in messages_data:
        messages.append({
            "room":message_data.room.id,
            "user":{
                "id":message_data.user.id,
                "username":message_data.user.username,
                "first_name":message_data.user.first_name,
                "last_name":message_data.user.last_name
            },
            "message":message_data.message,
            "msg_type":message_data.msg_type,
            "time":message_data.time.isoformat()
        })
    message.reply_channel.send({
        "text": json.dumps({
            "messages":messages,
...
def chat_send(message):
    ...
    new_message.save()
    room.send_message(message["message"], message.user, new_message.time.isoformat())

@channel_session_user
@catch_client_error
def chat_download(message):
    messages_data = Message.objects.filter(room__id=message['room'], time__lt=message['time']).order_by('-time')[:30]
    messages = []
    for message_data in messages_data:
        messages.append({
            "room":message_data.room.id,
...
def delete_room(message):
    room = Room.objects.get(pk=message['id']).delete()
    message.reply_channel.send({
        "text": json.dumps({
            "msg_type":9,
            "room":message['id'],
